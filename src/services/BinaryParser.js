...
export const BinaryParser= function (bigEndian, allowExceptions) {
...
p.encodeFloat = function (number, precisionBits, exponentBits) {
    ...
    var minExp = -bias + 1; 
    var maxExp = bias, minUnnormExp = minExp - precisionBits;
    ...
    var intPart;
    var floatPart;
    signal = (n = status !== 0 ? 0 : n) < 0, n = Math.abs(n); 
...
var pBuffer = (p.Buffer = function (bigEndian, buffer) {
            this.bigEndian = bigEndian || 0, this.buffer = [], this.setBuffer(buffer);
        }).prototype ;
         
pBuffer.readBits = function (start, length) {
...
pBuffer.setBuffer = function (data) {
    if (data) {
        for (var l, i = l = data.length, b = this.buffer = new Array(l); i; b[l - i] = data.charCodeAt(--i));
        this.bigEndian && b.reverse();
    }
};
pBuffer.hasNeededBits = function (neededBits) {
    return this.buffer.length >= -(-neededBits >> 3);
};
pBuffer.checkBuffer = function (neededBits) {
